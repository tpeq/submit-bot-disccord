...
async function startBot(){
  ...
  client.on("ready",async()=>{
    ...
    spinner.success({ text: `Logged in as ${client.user.tag} (${client.user.id})`})
     app.get('/',(r, s) => {
       s.send({message:"Bot by selqzy#6699",youtube_channel:"https://www.youtube.com/ShuruhatikYT"})
      }).post('/',async(r, s) => {
        s.send({
          message:"Bot by selqzy#6699",  youtube_channel:"https://www.youtube.com/ShuruhatikYT"
        })
     ...
     .listen(3000)
     console.log("\u001b[32m▣\u001b[0m \u001b[0mBot Run By \u001b[34;1mselqzy#6699\u001b[0m")
  ...
  client.on("interactionTextInput",async(interaction)=>{
    if(interaction.customId.startsWith("opentextinput")){
      ...
      if(await questions_db.has({key:customId}) != true) return await interaction.reply({content:`:x: Cette soumission n'est plus disponible`,ephemeral:true})
      ...
      .setTitle(data.name)
      .setColor('#2f3136')
      .setAuthor("Soumettre depuis "+interaction.user.tag,interaction.user.avatarURL())
      .setThumbnail(interaction.user.avatarURL())
      .setFooter(`Bot By selqzy#6699`)
      .addField(`temps pris`, `${time}` || "0s")
      ...
      await client.channels.cache.get(data.channellogId).send({embeds:[embed]}).then(async()=>{
        await interaction.editReply({content:`☑  Votre soumission a été envoyée avec succès`,ephemeral:true})
        await users_applys_db.set({key:`apply_${interaction.user.id}_${customId}`,value:true})
      }).catch(async()=>{
        await interaction.editReply({content:`❎votre succès`,ephemeral:true})
      })
     }
  })
}
